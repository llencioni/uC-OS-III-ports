...
// LRL - confirmar que nao vai dar merda retirar
...
// LRL
// Keil uV4 for LPC2129 is little-endian by default
...
typedef  CPU_INT32U  CPU_SIZE_T;            /* Defines CPU standard 'size_t'   size.                */
...
// LRL - stack grows from HIGH to LOW memory on ARM LPC2129
...
#define  CPU_CFG_CRITICAL_METHOD    CPU_CRITICAL_METHOD_STATUS_LOCAL	/* Configure CPU critical method (see Note #1) */

typedef  CPU_INT32U                CPU_SR;								/* Defines   CPU status register size (see Note #3b) */

//#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)	/* Allocates CPU status register word (see Note #3a) */
#define  CPU_SR_ALLOC()        CPU_SR  cpu_sr = (CPU_SR)0
#define  CPU_INT_DIS()         do { cpu_sr = OS_CPU_SR_Save(); } while (0)           /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { OS_CPU_SR_Restore(cpu_sr); } while (0)           /* Enable  interrupts.                          */
//#else
//#define  CPU_SR_ALLOC()
